...
# Will be played when an invalid move is made
wrong_sound = pygame.mixer.Sound("assets/sound effects/wrong_move.wav")
...
class Cube:
    ...
    def movement(self, event, tile_list, occupied_tiles, offset):
        ...
        if event.key in (pygame.K_w, pygame.K_UP):
            if (self.x, self.y + 1, self.z) in occupied_tiles:
        ...
        if event.key in (pygame.K_a, pygame.K_LEFT):
            if (self.x - 1, self.y, self.z) in occupied_tiles:
